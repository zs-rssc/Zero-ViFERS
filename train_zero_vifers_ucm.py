...
wandb.init(project='Zero-ViFERS', config='wandb_config/config_ucm.yml')
opt = wandb.config
...
netFE = model.Post_FE(opt, opt.attSize)
net_PFE = model.Pre_FE(opt, dataloader.w2v_att, dataloader.att,
...
optimizerFE = optim.Adam(netFE.parameters(), lr=opt.dec_lr, betas=(opt.beta1, 0.999))
optimizer_center = optim.Adam(center_criterion.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
optimizer_PFE = optim.RMSprop(net_PFE.parameters(), lr=0.0001, weight_decay=0.0001, momentum=0.9)
...
def generate_syn_feature(generator,classes, attribute,num,netFE=None):
...
def calc_gradient_penalty_FE(netFE, real_data, fake_data):
    ...
    interpolates = Variable(interpolates, requires_grad=True)
    _,_,disc_interpolates,_ ,_, _ = netFE(interpolates)
    ones = torch.ones(disc_interpolates.size())
    if opt.cuda:
        ones = ones.cuda()
    gradients = autograd.grad(outputs=disc_interpolates, inputs=interpolates,
...
for i in range(0, opt.pre_iters):
    net_PFE.train()
    optimizer_PFE.zero_grad()
    batch_label, batch_feature, batch_att = dataloader.next_batch(opt.pre_bs)
    out_package = net_PFE(batch_feature)

    in_package = out_package
    in_package['batch_label'] = batch_label
    
    out_package = net_PFE.compute_loss(in_package)
...
'''
    if i%100==0:
        ...
        acc_seen, acc_novel, H, acc_zs = util.eval_zs_gzsl(
            dataloader, net_PFE, opt.device, bias_seen=0, bias_unseen=0, batch_size=50)
...
'''
# refine features
data = util.DATA_LOADER_enhance(opt, net_PFE, dataloader)
#print("data", data)
...
for epoch in range(0,opt.nepoch):
    for loop in range(0,opt.feedback_loop):
        ...
        for i in range(0, data.ntrain, opt.batch_size):
            ...
            for p in netFE.parameters(): #unfreeze deocder
                p.requires_grad = True
            ...
            for iter_d in range(opt.critic_iter):
                ...
                ################# update FE
                netFE.zero_grad()
                muR, varR, criticD_real_FE, latent_pred, _, recons_real = netFE(input_resv)
                criticD_real_FE = criticD_real_FE.mean()
                R_cost = opt.recons_weight*WeightedL1(recons_real, input_attv) 
                
                fake = netG(z, c=input_attv)
                muF, varF, criticD_fake_FE, _, _, recons_fake= netFE(fake.detach())
                criticD_fake_FE = criticD_fake_FE.mean()
                gradient_penalty = calc_gradient_penalty_FE(netFE, input_resv, fake.data)
                center_loss_real = center_criterion(
                    muR, input_label, margin=opt.center_margin, incenter_weight=opt.incenter_weight)
                D_cost_FE = center_loss_real*opt.center_weight + R_cost
                D_cost_FE.backward()
                optimizerFE.step()
            ...
            if opt.recons_weight > 0 and opt.freeze_dec:
                for p in netFE.parameters(): #freeze decoder
                    p.requires_grad = False
            ...
            ######################################original
            netFE.zero_grad()
            _,_,criticG_fake_FE,latent_pred_fake, _, recons_fake = netFE(fake, train_G=True)
    ...
    netFE.eval()
    # netF.eval()
    syn_feature, syn_label = generate_syn_feature(netG,data.unseenclasses, data.attribute, opt.syn_num,netFE=netFE)
    ...
    if opt.gzsl:  
        if opt.final_classifier == 'softmax':
            ...
            gzsl_cls = classifier_zero.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5,
                                                  ...
                                                  netFE=netFE, dec_size=opt.attSize, dec_hidden_size=(opt.latensize*2), opt=opt)
