...
class DATA_LOADER_enhance(object):
    def __init__(self, opt, net_PFE, dataloader):
        self.opt = opt
        self.dataloader = dataloader
        self.device = opt.device
        self.net_PFE = net_PFE
        self.read_matdataset(opt, self.pre_enhance_features(
            net_PFE, dataloader, opt.device))
        self.index_in_epoch = 0
        self.epochs_completed = 0

    def read_matdataset(self, opt, PreFE_features_dict):
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat")
        label = matcontent['labels'].astype(int).squeeze() - 1
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + "_splits.mat")
        trainval_loc = matcontent['trainval_loc'].squeeze() - 1
        ...
        self.train_feature = PreFE_features_dict['train_feature']
        self.train_label = torch.from_numpy(label[trainval_loc]).long() 

        self.test_unseen_feature = PreFE_features_dict['test_unseen_feature']
        self.test_unseen_label = torch.from_numpy(label[test_unseen_loc]).long() 

        self.test_seen_feature = PreFE_features_dict['test_seen_feature']
        self.test_seen_label = torch.from_numpy(label[test_seen_loc]).long()
    ...
    def pre_enhance_features(self, model, dataloader, device, extract_batch_size=50):
