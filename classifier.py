...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5,
                 ...
                 netFE=None, dec_size=4096, dec_hidden_size=4096, opt=None):
        self.opt = opt
        ...
        if self.netFE:
            ...
            elif self.opt.feature_component == 'FR_l':
                self.input_dim = self.input_dim + dec_hidden_size
            elif self.opt.feature_component == 'FR_l_r':
                self.input_dim = self.input_dim + dec_hidden_size + dec_size
    ...
    def compute_fear_out(self, test_X, new_size):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                ...
                with torch.no_grad():
                    inputX = Variable(test_X[start:end])
            _,_,_,_, _, feat2 = self.netFE(inputX)
            feat1 = self.netFE.getLayersOutDet()
