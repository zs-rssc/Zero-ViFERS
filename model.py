...
class Post_FE(nn.Module):
    def __init__(self, opt, attSize):
        super(Post_FE, self).__init__()
        self.embedSz = 0
        self.hidden = None
        self.lantent = None
...
class Pre_FE(nn.Module):  
    def __init__(self, opt, init_w2v_att, att, seenclass, unseenclass):  
        ...
        self.att = F.normalize(att.clone()).to(device)
    ...
    def extract_attention(self, Fs):
        ...
        #print("shapes of Vn W2 Fs", V_n.shape, self.W_2.shape, Fs.shape)
    ...
    def compute_attribute_embed(self, Hs):
        ...
        V_n = F.normalize(self.V) if self.normalize_V else self.V
        #print("Hs, V_n shapes:", Hs.shape, V_n.shape)
